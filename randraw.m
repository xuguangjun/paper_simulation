     ...
     if prod(sampleSize) > 0
          switch lower( distribNameInner )
               ...
               case {'zeta', 'zipf'}
                    ...
                    %    "Non-Uniform Random Variate Generation,"
                    %    Springer Verlag, 1986, pages 550-551.
                    % zipf分布随机数采用蒙特卡罗方法，N个数据的概率加起来综合为1（这里将N定为100，即有100部电影），然后求每个数据的概率区间
                    % 产生0-1之间的随机数来等同概率，反推数据n。
                    ...
                    %u1 = rand( sampleSize );%generate a sampleSize random from the standard uniform distribution
                    %out = floor( u1.^(-1/am1) );%B = floor(A) rounds the elements of A to the nearest integers less than or equal to A
                    %clear('u1');
                    %u2 = rand( sampleSize );
                    %t = ( 1 + 1./out ).^(a-1);

                    %indxs = find( u2.*out.*(t-1)/bm1 > t/b );%find non-zero element
